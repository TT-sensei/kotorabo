...
function header() {
  return `<header class="site-header">
    ...
    <nav aria-label="メインメニュー"><button data-action="home" class="${state.screen === "home" ? "active" : ""}">今日のチャレンジ</button><button data-action="carte" class="${state.screen === "carte" ? "active" : ""}">できたこと</button><span class="grade-chip">小学${state.profile.grade}年</span></nav>
  </header>`;
}
...
function resultTemplate() {
  ...
  return `<main class="center-page result-page"><section class="result-card"><div class="discovery-burst">🎉</div><p class="eyebrow">チャレンジ クリア！</p><h1>ことばパワーがアップ！</h1><div class="result-numbers"><div><strong>${records.length}</strong><span>考えた問題</span></div><div><strong>${best}</strong><span>一番ぴったり</span></div><div><strong>${recovered}</strong><span>考え直してできた</span></div></div>${skills.length ? `<div class="discoveries"><h2>できるようになってきたよ</h2>${skills.slice(0,3).map((skill) => `<span>✦ ${skill}</span>`).join("")}</div>` : ""}${tryPrompt ? `<div class="try-it"><small>やってみよう！</small><p>${tryPrompt}</p><span>正解・不正解はないよ。声に出したり、短く書いたりしてみよう。</span></div>` : ""}<div class="button-row"><button class="secondary-button" data-action="again">もう一回！</button><button class="primary-button" data-action="home">ホームへ <span>→</span></button></div></section></main>`;
}
